	...
	const parseUser = (userJson: string | null): UserProfile | null => {
		if (userJson !== null) {
			try {
				...
				const currentUser: UserProfile = JSON.parse(user);
				return ({ userId: currentUser["userId"], fullname: currentUser["fullname"], email: currentUser["email"], avatar: currentUser["avatar"] });
			} catch {
				return null;
			}
		}
